...
class AudioManager {
  ...
  async playOverlayMusic(name, fadeSecs = 0.5) {
    ...
    this._duckBG('minigame', 0.12, fadeSecs);
    ...
    if (this.useWebAudio) {
      ...
      if (buffer) {
        ...
        // Si había overlay anterior, pararlo
    ...
    if (this.overlayMusicFallback) {
      this.overlayMusicFallback.pause();
      this.overlayMusicFallback = null;
    }
    const audio = new Audio(url);
    audio.loop   = true;
  ...
  stopOverlayMusic(fadeSecs = 0.5) {
    ...
    if (this.useWebAudio) {
      if (this.overlayMusicGainNode && this.overlayMusicSource) {
        const now = this.ctx.currentTime;
        ...
        setTimeout(() => { try { os.stop(); os.disconnect(); } catch(e) {} }, fadeSecs * 1000 + 50);
        this.overlayMusicGainNode = null;
        this.overlayMusicSource   = null;
      }
    } else {
      if (this.overlayMusicFallback) {
        ...
        const start = old.volume || 0;
        const steps = Math.max(1, Math.round(fadeSecs * 20));
        let n = 0;
        const iv = setInterval(() => {
          n++;
          old.volume = Math.max(0, start * (1 - n / steps));
          if (n >= steps || old.volume <= 0.001) {
            clearInterval(iv);
            old.pause();
            old.currentTime = 0;
          }
